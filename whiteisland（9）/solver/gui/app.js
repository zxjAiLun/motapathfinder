...
function renderControls() {
  ...
  $("controls").innerHTML = `
    <button id="btn-start" ${canStart ? "" : "disabled"}>Start Live</button>
...
function renderTimeline() {
  ...
  const rows = (route.decisions || []).map((decision) => {
    ...
    const item = decision.enemyId || decision.itemId || decision.tool || decision.equipId || decision.doorId || "";
...
async function startLive() {
  ...
  state.session = await postJson("/api/session/start", { fromStep, stepDelayMs: state.speedDelayMs });
  ensurePolling();
  renderAll();
}
