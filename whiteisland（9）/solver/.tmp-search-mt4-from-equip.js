...
const startRecord=JSON.parse(fs.readFileSync('routes/latest/continue-hp3779-i893.route.json','utf8')); let start=sim.createInitialState({rank:'chaos'});
...
const startLen=start.route.length; console.log('start',start.floorId,start.hero,'routeLen',startLen);
...
for(let depth=0; depth<=28; depth++){
 ...
 if(goal)break; next.sort((a,b)=>posScore(b)-posScore(a)); frontier=next.slice(0,2500); console.log('depth',depth,'frontier',frontier.length,'best',best.floorId,best.hero.hp,best.hero.loc);
}
