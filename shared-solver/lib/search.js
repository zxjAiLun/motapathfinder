...
function compactAction(action, project) {
  ...
  if (action.kind === "resourcePocket" || action.kind === "resourceChain" || action.kind === "resourceCluster") {
    ...
    compact.resourcePocket = {
      hpAfter: action.estimate && action.estimate.hpAfter,
      atk: action.estimate && action.estimate.atk,
      def: action.estimate && action.estimate.def,
      mdef: action.estimate && action.estimate.mdef,
      exp: action.estimate && action.estimate.exp,
...
function recordDroppedActionSample(stats, reason, state, action) {
  ...
  pushLimitedSample(stats.droppedProgressActions.samples, {
    ...
    action: compactAction(action, stats.project),
  }, stats.sampleLimit);
}
...
function ensureConfluenceStats(stats, config) {
  if (!stats) return null;
...
function registerConfluenceState(simulator, confluenceBestByKey, state, stats, config, commit) {
  ...
  nextList.push(state);
  nextList.sort((left, right) => simulator.compareSearchConfluenceStates(left, right));
  confluenceBestByKey.set(key, nextList.slice(0, Number(config.confluenceRepresentatives || 2)));
  return true;
}
...
function rebuildFrontierState(simulator, frontier, expandedStateKeys, options) {
  ...
  const confluenceBestByKey = new Map();
  const rebuiltFrontier = [];

  sortFrontier(simulator, frontier, options);
  frontier.forEach((state) => {
    if (!registerState(simulator, activeRecords, dominanceBuckets, bestByDominanceKey, expandedStateKeys, state, null, options, confluenceBestByKey)) return;
...
function registerState(simulator, activeRecords, dominanceBuckets, bestByDominanceKey, expandedStateKeys, state, stats, options, confluenceBestByKey) {
  ...
  if (!registerConfluenceState(simulator, confluenceBestByKey, state, stats, config, false)) {
    if (stats) stats.perf.timeInDominanceMs += nowMs() - startedAt;
    return false;
  }
...
function maybeAutoExpandForwardChangeFloors(simulator, parentNode, parentState, frontier, heap, activeRecords, dominanceBuckets, bestByDominanceKey, expandedStateKeys, stats, config, updateBest, registerChild) {
  ...
  actions.slice(0, 3).forEach((action) => {
    ...
    childState.__sourceAction = compactAction(action, simulator.project);
...
function searchTopKSerial(simulator, initialState, options) {
  ...
  let confluenceBestByKey = new Map();
  ...
  const registerChild = (parentNode, childState, action) => {
    ...
    if (!registerState(simulator, activeRecords, dominanceBuckets, bestByDominanceKey, expandedStateKeys, childState, stats, config, confluenceBestByKey)) return null;
  ...
  registerState(simulator, activeRecords, dominanceBuckets, bestByDominanceKey, expandedStateKeys, initialState, stats, config, confluenceBestByKey);
  ...
  try {
    while (heap.size > 0 && expansions < maxExpansions) {
      ...
      if (shouldTrim()) {
        ...
        confluenceBestByKey = rebuilt.confluenceBestByKey;
      }
      stats.perf.maxFrontierSize = Math.max(stats.perf.maxFrontierSize, frontier.length);
    }
  } finally {
