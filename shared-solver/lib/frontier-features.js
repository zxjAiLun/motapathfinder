...
function estimateBattleUnlockOpportunity(project, state, battleResolver, floorId, startX, startY, initialDamage) {
  const floor = project.floorsById[floorId];
  if (!floor || !floorHasCoordinate(project, floorId, startX, startY)) {
    return {
      score: 0,
      itemCount: 0,
      changeFloorCount: 0,
      zeroDamageChainCount: 0,
      lowDamageBattleCount: 0,
    };
  }
  ...
  const visited = new Set([coordinateKey(startX, startY)]);
  const seenItems = new Set();
  const seenChangeFloors = new Set();
  const seenEnemies = new Set([coordinateKey(startX, startY)]);
  let score = Math.max(0, 120 - Number(initialDamage || 0) * 2);
  let itemCount = 0;
  let changeFloorCount = 0;
  let zeroDamageChainCount = Number(initialDamage || 0) === 0 ? 1 : 0;
  let lowDamageBattleCount = Number(initialDamage || 0) <= 25 ? 1 : 0;
  ...
  while (queue.length > 0) {
    ...
    DIRECTIONS.forEach((direction) => {
      ...
      if (changeFloor && !seenChangeFloors.has(locKey)) {
        seenChangeFloors.add(locKey);
        changeFloorCount += 1;
        score += changeFloor.floorId === ":next" ? 900 : 350;
      }

      const tile = getTileDefinitionAt(project, previewState, floorId, x, y);
      if (tile && tile.cls === "items" && (tile.trigger == null || tile.trigger === "getItem") && !seenItems.has(locKey)) {
        seenItems.add(locKey);
        itemCount += 1;
        score += getItemPreviewUtility(project, previewState, floorId, tile.id);
      }

      if (isEnemyTile(tile)) {
        if (seenEnemies.has(locKey)) return;
        seenEnemies.add(locKey);
        const battle = evaluateBattleFrontier(project, previewState, battleResolver, floorId, x, y, tile);
        if (!battle) return;
        const damage = Number(battle.damageInfo.damage || 0);
        if (damage === 0) {
          zeroDamageChainCount += 1;
          score += 180;
          ...
          lowDamageBattleCount += 1;
          score += Math.max(25, 90 - damage * 2);
        }
        return;
      }

      if (!isTargetTraverseTile(project, previewState, floorId, x, y)) return;
      if (visited.has(locKey)) return;
      visited.add(locKey);
      queue.push({ x, y });
    });
  ...
  return {
    ...
    itemCount,
    changeFloorCount,
    zeroDamageChainCount,
    lowDamageBattleCount,
  };
}
...
function computeFrontierFeatures(project, state, options) {
    ...
    DIRECTIONS.forEach((direction) => {
      ...
      if (isEnemyTile(tile)) {
        ...
        if (battle) {
          ...
          record.preview = estimateBattleUnlockOpportunity(project, state, battleResolver, state.floorId, x, y, record.damage);
          battleFrontier.set(locKey, record);
          battleOpportunities[locKey] = record.preview;
          battleDamages.push(record.damage);
  ...
  const features = {
    ...
    minBattleDamage: battleDamages.length > 0 ? Math.min(...battleDamages) : null,
    maxBattleDamage: battleDamages.length > 0 ? Math.max(...battleDamages) : null,
    minBattleDamageRatio: battleDamages.length > 0 && Number(state.hero.hp || 0) > 0
      ? Math.min(...battleDamages) / Number(state.hero.hp || 0)
      : null,
    maxBattleDamageRatio: battleDamages.length > 0 && Number(state.hero.hp || 0) > 0
      ? Math.max(...battleDamages) / Number(state.hero.hp || 0)
      : null,
