...
function defaultSearchRank(state, score, context) {
  ...
  const preferredChangePreview = frontier && frontier.preferredChangeFloor && frontier.preferredChangeFloor.preview
    ...
    : null;
  const atk = Number(state.hero.atk || 0);
  const def = Number(state.hero.def || 0);
  const mdef = Number(state.hero.mdef || 0);
  const hp = Number(state.hero.hp || 0);
  const rareKeyReserve = Number((state.inventory || {}).blueKey || 0) * 100 + Number((state.inventory || {}).redKey || 0) * 60;
  return {
...
function compareSearchRank(left, right) {
  ...
  if (left.coreCombat !== right.coreCombat) return left.coreCombat - right.coreCombat;
  if (left.resourcePrepScore !== right.resourcePrepScore) return left.resourcePrepScore - right.resourcePrepScore;
  if (left.survival !== right.survival) return left.survival - right.survival;
  if (left.rareKeyReserve !== right.rareKeyReserve) return left.rareKeyReserve - right.rareKeyReserve;
  if (left.hp !== right.hp) return left.hp - right.hp;
  if (left.mdefValue !== right.mdefValue) return left.mdefValue - right.mdefValue;
