...
function selectOracleFloorResults(results, maxEntries) {
  ...
  candidates
    .slice()
    .sort((left, right) => summarizeHero(right.state).hp - summarizeHero(left.state).hp)
    .forEach(add);
  return selected;
}
...
function tryReachAndBattle(simulator, state, target, segment, config, oracleCache, stats) {
  ...
  if (stats) stats.successorCandidatesBeforeCap = Number(stats.successorCandidatesBeforeCap || 0) + results.length;
...
function buildMonsterOnlyActionProvider(simulator, segment, config, stats) {
  ...
  return (unusedSimulator, state) => {
    ...
    if (stats) {
      ...
      if (targets.length > maxTargets) stats.statesWithTargetCap = Number(stats.statesWithTargetCap || 0) + 1;
    }
