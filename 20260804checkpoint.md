...
- **PR-5.5c Continuation（完成，研究/证据轮，零 production 行为改动）**：High-Semantic-Diversity Collision Hunt。**P2-3 修正**：deep fixture（exp6-entryA-deep）实测 post 10→10 无增量（26 样本实为 r0-exp2-entryB），已删除，替换为 mutation-divergence workload（r0-tile2_1-entryA：tileRemoved(2,1) 携带与 tile4_1 不同的 floor-mutation 历史）。新增 **semantic-diversity report**（每个 workload + 全局：regionKey / reachableEndpoints / mutationSummary / inventory / flags / visitedFloors / legalActionSet / loc 的 distinct 值计数，从已计算的 production key JSON 读取，零重模拟）+ **semantic gate**（每 workload ≥1 维度 ≥2 distinct；全局 mutation/reachability/flags/legalActions ≥2）。**关键发现**：全局 inventory=1、visitedFloors=1——auto-pickup 立即捡光 MT1 所有物品、且 corpus 是单 floor，inventory 在 MT1 corpus 中语义恒定（如实报告为 narrowness finding，不伪装成多样性）。结果 **NO_COLLISION_OBSERVED**（18 workload，42 pre / 42 boundary / 181 post，0 merge / 0 unsafe）——证据面在语义维度上真实量化，仍无真实同 scope candidate collision。
