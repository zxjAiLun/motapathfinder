...
- **PR-5.16a Search Observatory Baseline（完成，提交见本文件 HEAD）**：把 blind search 的 objective、自动 feasibility hypotheses、candidate funnel、budget ledger、goal-rank plateau、human-review candidates、next falsifiable experiment 与 same-control comparison 合为一个报告。D2/1000：Boss/I894 candidate 均 0；641 MT5 + 359 MT4 expansions；599 battle + 400 changeFloor incoming；3032 generated、1490 accepted successors（1491 registered states 含 root）、1542 rejected、frontier202；最大 rank tuple 343。报告明确 `wasteProven=false`，不把高频/被拒/换层自动叫浪费；只有同控制 A/B 才能形成因果优化结论。冻结 `SEARCH_INTENT_AND_COST_BASELINE_VISIBLE`，production search semantics 不变。
- **PR-5.16b Automatic AND/OR Dependency Planner（完成，提交见本文件 HEAD）**：修复 automatic graph 对相邻 POI 没有 contact edge 的静态断图，409 nodes / 722→906 edges；D2/I894 自动生成 2 个 OR alternatives，内部 5/7 个 AND blockers，共同前置为 MT5:10,5 goldHornSlime。真实 current-state battle probes 区分 viable/unbeatable/lethal（最大显式 lethal damage 211957）；移除 contact edge 后 disconnected、无 selected subgoal 均 fail-visible。Observatory 已展示 dependency board，下一轮转 local DP multi-role checkpoint executor。冻结 `AUTOMATIC_AND_OR_DEPENDENCY_PLAN_COMPILED`，未改 production 搜索语义。
...
- **PR-5.17a3 Least-Commitment Checkpoints + Reversible Repair Trials（完成，提交见本文件 HEAD）**：重新审视 5.17a2 后发现“缺 EXP”不是首个可靠结论：goal skyline 曾忽略 expansion-1 的 first goal，反馈又优先眼前高 HP，导致在未证明必要时提前消耗一次性资源；blocker repair 的 pickup counterfactual 也没有证明真实可达。局部 executor 现显式保留/标记 first-goal checkpoint，上层优先 least-commitment 出口，并保存 root/history portfolio 做 OR 回溯。修复子目标作为连续事务执行到实际 pickup；若反事实候选真实执行完整穷尽仍失败，则以 `blocker-repair-rejected` 记录并恢复原 portfolio，同一 state/resource experiment 不再重试。固定 6-round checker 中 expansion-1 依次推进 MT5 两个 skeletonKing 与 skeletonPresbyter；I621 试验以 **9 expansions / frontier0 / searchComplete=true / goalFound=false** 被否决，随后未污染状态地继续完成 I1014。总计 **220 expansions / 694 generated / 488 accepted / 206 rejected**；相对 5.17a2 的 156 expansions 不是速度改进，而是新增 64 expansions 购买了真实 save/load 式否证与继续搜索能力。D2 仍 OPEN；下一轮先把 counterfactual benefit 改为 acquisition-cost 后的 actual net margin，再缓存重复 graph/dependency compilation。
- **PR-5.17a4 Actual Net-Margin Observatory + Graph Reuse（完成，提交见本文件 HEAD）**：每次 repair 完成后，使用真实取得资源后的 checkpoint 重新评估原 combat blocker，并并列 predicted/actual。I1014 的免费 pickup 预测为 damage93931、margin **+3583**；真实路径先支付 devilWarrior **65810 HP**，实际 damage 不变但 margin **-62227**，prediction error **-65810**，因此 closure 明确为 `improved-but-still-blocked`，而非“已解除 blocker”；相对 repair 前 margin 仍净改善 72190。性能侧复用每个 checkpoint 已构建的 macro graph：固定 8 checkpoints / 9 unique access probes，graph builds **17→8**、reuse **0→9**，selected experiment/candidate count 不变。wall time 保持辅助观测，不用于宣称因果加速。D2 仍 OPEN；下一轮让实际 closure class 回流候选排序/组合修复，并继续记录每步净成本。
- **PR-5.17a5 Actual Closure Commit/Rollback + Streaming Trace（完成，提交见本文件 HEAD）**：actual closure class 现在直接决定事务提交。`blocker-unblocked` 与 `improved-but-still-blocked` 保留真实 checkpoint 继续组合；`no-net-improvement`、`repair-target-not-realized` 或无 checkpoint 则拒绝 experiment 并恢复输入 portfolio。14-round A/B 中 I642 的 free-pickup 预测 margin **+22687**，真实获取后 margin **-27998** 且相对输入无净改善；旧版 round13 接受并污染状态，新版同轮输出 `blocker-repair-rejected / no-net-improvement`，round14 从历史 checkpoint 继续。probe 新增逐轮 NDJSON：每轮完成立即显示 elapsed/round wall、动作/repair、predicted/actual margin、closure、expansions 与 phase timing。首三段实测 DP search **1.2–1.7s**，8-checkpoint record+strict replay **2.8–5.4s**，dependency evaluation **0.9–1.0s**；这证明当前单轮主要 wall 成本在多 checkpoint 证据构造/回放，不在 32/64 expansions 本身。D2 仍 OPEN；下一轮优化 replay 证据流水线，但不得降低最终 strict replay gate。
- **PR-5.17a6 Shared Simulator Replay Pipeline（完成，提交见本文件 HEAD）**：phase timing 指向 checkpoint record/replay 后，先消除无语义价值的 simulator 重建。旧实现对 8 个 checkpoints 每个分别为 route record 与 strict replay 新建 simulator，加上 DP 本体共 **17 instances**；新实现由同一 local execution 共享单个 simulator，变为 **1 instance**。同输入 A/B 保持 64 expansions、8 checkpoints、8/8 strict replay、全部 exact-state/route fingerprints 与选择完全一致；顺序实测 wall **8.866s→5.191s（方向性 -41.4%）**。构造计数与结果等价是因果证据，单次 wall 仅辅助。没有减少 checkpoint、取消 replay 或修改 production key/dominance/selection。D2 仍 OPEN；下一轮继续组合链并寻找第一个 actual `blocker-unblocked`。
- **PR-5.17a7 Semantic Repair Experiment Identity（完成，提交见本文件 HEAD）**：30-round trace 发现 I642 有两组逐字相同的 predicted/actual 结果，却因 exact checkpoint/navigation identity 不同而重复执行。输入差异审计锁定 round18/19 的资源、背包、真实 flags、visited floors、地图 mutation、access plan 与 blocker evidence 全同，唯一投影差异是 `__leaveLoc__.MT4` 的离层坐标/方向。新 identity 保留资源状态、真实 flags、visited/mutation、acquisition access 与 blocker evidence，只忽略 hero loc/direction 及 `__leaveLoc__` 导航缓存。相同控制下 repair trials **22→20**、两组相邻重复 **2→0**；expansions **775→821**，因为释放轮次继续探索，不宣称纯加速。actual unblock 仍 **0**，D2 OPEN；下一轮处理每轮重复 macro graph/access compilation wall 成本。
- **PR-5.17a8 Exact-State Repair Compilation Cache（完成，提交见本文件 HEAD）**：同一 run 内按 exact state 缓存 automatic macro graph + feasibility/dependency analysis，并按 exact state/resource 缓存 access summary；不复用 state/simulator/候选决定。6-round cache-off/on 的 repair 顺序、predicted/actual margins 与 **220 expansions / 694 generated / 488 accepted / 206 rejected** 全同。graph builds **25→17**、graph-backed access compiles **87→68**，cache hits 为 **8 analysis + 19 access**；compilation wall **23.380s→18.718s（方向性 -19.9%）**。I621 rollback 后下一轮同 5 checkpoints 为 0 builds/5 hits。D2 仍 OPEN；下一轮检查 counterfactual candidate scoring 与 history scheduling。
- **PR-5.17a9 Combat-Reward Progress Repair（完成，提交见本文件 HEAD）**：automatic resource vocabulary 加入当前状态 simulator 可打的 enemy；counterfactual 真实支付 battle damage、应用 EXP/升级/事件，再将即时 blocker 改善与 level progress 分开。round13 I642 no-net 后，旧控制 round14 historical backtrack；新控制 9 expansions 获取 `MT4:10,5 devilWarrior` combat reward，actual delta **-42685 HP/+20 ATK/+20 DEF/+200 MDEF/+26 EXP**，Boss margin 仍 -27998，closure 仅 `progressed-toward-level-threshold`。随后 180 item + 36 combat candidates、72 unique access 全 non-startable，首失败面推进到复合跨 gate EXP chain。D2 OPEN。
- **PR-5.17a10 Failure-Driven Progress Priority（完成，提交见本文件 HEAD）**：combat progress 后若当前 portfolio 无可启动 repair，历史回溯将候选模式切到 `level-progress-first`，真实升级或 blocker-unblocked 后才恢复。fixture 同 checkpoint 的默认选择为 I1014，进展模式改为 `MT4:8,11 poisonBat/+16 EXP`。20-round 从 round16 起连续选择 combat；50-round 共 27 次 combat repair，但总 expansions **1215→1229**、actual unblock 仍0，最终 frontier 仅 **lv7/exp263或289**。这证明选择已从 item 转向 EXP，却在不同历史分支间反复试验、没有单调累积同一条升级链；repair compilation 累计 **144.410s / 700 unique access probes**。D2 OPEN，下一轮修 cumulative-progress portfolio selection，不加轮数。
- **PR-5.17a11 Cumulative Progress + Level-Threshold Probe（完成，提交见本文件 HEAD）**：level-progress 模式下 history 按 level/EXP/route/recency，combat candidates 按 counterfactual `levelAfter/expAfter`；默认 blocker-first 不变。20-round 路线最多 11→15 decisions，但 max EXP 仍289，说明 tie-break 不是 closure。控制器随后自动生成 `minHero.lv=8` 复合目标；三个不同 exact state 均为 lv7/exp315，全部 **9 expansions / frontier0 / searchComplete=true / goalFound=false**。因此当前存档已无可执行升级链，必须向更早 history 倒推 EXP/resource dependency。总量 **489/1359/981/378**，D2 OPEN。
- **PR-5.17a12 Replay-Verified Partial Progress（完成，提交见本文件 HEAD）**：history level probes 先按当前可执行 battle EXP 潜力排序并限制4次；局部 lv8 search 即使未完成，也可保留 absolute level/EXP 严格上升的单一 best-progress，但必须 route record + strict replay。真实 contract MT5 entry exp77→173（32 expansions/frontier57/goalFound=false/6 decisions strict）；集成旧行为丢弃 exp205→221，新行为提交后继续 combat +94→exp315。partial objective 期间 terminal dependency 不抢占。exp315 仍完整穷尽，故 leveling 不是已证 closure，D2 OPEN。
- **PR-5.17a13 Deferred-Heal First-Divergence Attribution（完成，提交见本文件 HEAD）**：tracked success 只作 post-hoc oracle，不进入 planner 输入。同一 common state 的单变量 A/B 锁定 `battle:skeletonKing@MT4:8,3` 的 I621 消费时机：早吃与晚吃最终 ATK/DEF/MDEF/LV/EXP 完全相同，晚吃多 **13,846 HP**；到 evilHero 前 damage 同为60,453，margin 从 **-11,108→+2,738**，直接由 lethal 翻为 viable。自主 planner 自己发现 I621，但只生成 `tileRemoved/consume-now`，没有 preserve/defer intent；到该错误语义已累计201 expansions/657 generated，且随后9 expansions完整穷尽仍失败。冻结 `HIERARCHICAL_REPAIR_MISSING_DEFERRED_RESOURCE_INTENT`；5.17a14 只修 feedback→local-goal 的一般化时序接口，不改 DP key/dominance，不写死 I621。
- **PR-5.18a Strategic D2 Vertical Slice（完成，提交见本文件 HEAD）**：停止 5.17a14 局部 repair，新增独立战略 frontier。物品/怪物二维存在表由 base grid + sparse mutations 共享物化；64-expansion real D2 为272 generated/204 accepted/68 exact merge/frontier141、54 maps、depth15，六队列全部实际展开，并自动记录 MT4:8,3→I621 的隐式消费。floorFly 控制在8 nodes 中占1105/1134 variants，故 minimal slice 默认延期该 supplemental transport、保留完整 primitive+interactPickup。1000-expansion为 false/false/true/false、frontier825、32.310s、terminal action 0；旧 blind DP 同预算 false/frontier202/22.537s。新架构尚未赢，当前卡点转为一般化战略评价：goal/power 偏向 exp315死状态、novel 退回MT2、resource-options 几乎不提交。D2 OPEN。
- **PR-5.18b StrategicTransition Contract（完成，提交见本文件 HEAD）**：raw variants 按 path-independent choice 聚合，保留 reachable-POI/机会成本/不可逆成本/terminal blocker progress；六队列改为 blocker/survival/power/future-options/low-cost/semantic-novelty。correctness 收紧：reachable 只按 exact state 缓存，door/event/portal 存在性查真实 after-state，novelty 按 lineage 新发现 POI；Boss damage 不可算时用 ATK 缺口，1000-expansion 从 -1223 改善到最好 -923、1025 transitions 有 improvement，但 terminal action 仍0。正式同预算 false/false/true/false、deferredWorkRemaining=true、48.562s、frontier1119、3342 choices/3749 variants、2118 accepted、1224 exact merge、407 deferred。D2 OPEN；deferred post 未解析前不得声称 complete。
...
7. **PR-5.15b 自动子目标（已完成）**：机器已从扩展 envelope + 当前地图资源 + Boss stats 自动选出 deficit-reducing equipment goal；但原 DP 无 predecessor guidance，1000 expansions 仍未到达。
8. **PR-5.16a 可观察性（已完成）**：每轮搜索先公开机器的假设、候选漏斗、预算账本和未证明的低收益嫌疑。
9. **PR-5.16b AND/OR dependency planner（已完成）**：静态拓扑候选、当前 simulator feasibility 与 AND/OR 替代关系已可见；不得把 alternative-1 写回人工 milestone。
...
14. **PR-5.17a3 reversible repair trials（已完成）**：先纠正此前过早的“缺 EXP”归因。first-goal/least-commitment checkpoint 防止无理由提前消费；counterfactual repair 必须经过真实 local DP 获取，失败则回滚并对 state/resource pair 去重。下一轮对每个候选报告 `predicted margin → acquisition cost → actual net margin`，再用同控制比较决定评分是否改善。
15. **PR-5.17a4 actual net margin（已完成）**：`predicted viable` 不再等于 `actual unblocked`；获取战损进入实际 blocker 复验，并产出 closure class。macro graph 的结构复用以 build count 17→8、输出不变锁定。下一轮把 `improved-but-still-blocked` 作为可组合中间状态，不重复宣称 closure。
16. **PR-5.17a5 actual closure commit/rollback（已完成）**：actual no-net repair 必须回滚；stream trace 必须逐轮输出，避免长运行结束前完全不可见。phase timing 已将当前优化重点从 DP expansion 转到 checkpoint record/strict replay pipeline。
17. **PR-5.17a6 shared simulator replay（已完成）**：单一 local execution 的 simulator 可在 DP、route record 与各 checkpoint strict replay 间复用；A/B 必须锁住全 fingerprints 与 8/8 replay。不得把单次 wall 当作唯一性能证据。
18. **PR-5.17a7 semantic repair experiment identity（已完成）**：相同资源状态 + acquisition access + blocker evidence 的 repair 只真实试一次；`__leaveLoc__`/hero loc 不得制造新实验，真实 gameplay flag 或 access plan 改变则必须视为新实验。重复归零不等于路线解除。
19. **PR-5.17a8 exact-state repair compilation cache（已完成）**：只在单次 hierarchical run 内复用 immutable graph/analysis/access summary；cache off/on 必须保持 repair key、margin、搜索计数与 checkpoint 结果不变。wall 仅作方向性辅助。
20. **PR-5.17a9 combat-reward progress repair（已完成）**：combat reward counterfactual 必须走真实 battle/reward/level-up 语义；EXP progress 可作为可回滚中间状态，但只有 `blocker-unblocked` 才能声明 gate closure。下一轮解决全部单资源 access blocked 的复合链。
21. **PR-5.17a10 failure-driven progress priority（已完成）**：失败信号可以切换资源类别优先级，但不能只看每次 acquisition delta；必须比较 repair 后 portfolio 的绝对 level/EXP 与 lineage，持续选择真正累积进展的存档。下一轮冻结相同 50-round 控制验证最终 max EXP/level 是否提升。
22. **PR-5.17a11 cumulative progress + level threshold（已完成）**：绝对 level/EXP 排序只能纠正分支选择，不能让已经 frontier-exhausted 的 blocked state 重新产生经验来源。自动 `lv+1` goal 的完整失败必须作为 backward dependency witness；下一轮在更早 checkpoints 搜索能达到该阈值的组合链。
23. **PR-5.17a12 replay-verified partial progress（已完成）**：best-progress 只有在绝对 level/EXP 上升时才值得支付 replay 成本并提交；partial transaction 必须保持 objective continuity。exp205→221→315 是机制成功，不是 D2 成功；下一轮停止把 lv8 当唯一侧重，审计 blocker 前成功/失败状态的资源与事件差异。
24. **PR-5.17a13 deferred-resource attribution（已完成）**：搜索已能发现 I621，但把它误写成“现在获取”的 repair；成功所需的是“现在保留、越过成本链、以后获取”。该证据只用于否证旧 hierarchical vocabulary，不再授权 5.17a14 专项 repair。
25. **PR-5.18a strategic D2 vertical slice（已完成）**：战略节点从完整 primitive POI transition 的 exact post state 产生，地图 option grid 必须共享而非逐节点复制；heuristics 只在六队列调度，不进入剪枝。floorFly variant explosion 与 terminal-action=0 已可见。下一轮先修 future-option/blocker reachability 评价和无效跨层 novelty，不扩大预算、不读取 oracle 顺序。
26. **PR-5.18b StrategicTransition contract（已完成）**：choice aggregation 与 transition diagnostics 已成立，但每个 choice 的非 canonical exact posts 目前只是记录、未展开，属于显式 completeness limitation。5.18c 接 local DP connector，并惰性恢复 deferred post/travel/floorFly；验收必须让这些工作进入 frontier 或给出安全等价证明，不能继续只调评分。
