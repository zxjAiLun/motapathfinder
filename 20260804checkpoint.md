...
1. **PR-5.9a 已关闭；下一轮进入 PR-5.9b safe-fast skeleton cache**：每个 exact state 保留独立 safety classification；只复用 `floor/start/current-floor mutations` 的无 state topology/path skeleton，命中后 rebase 当前 state。A/B 必须锁 action/successor exact、winner/route/objective/expansions 与 strict replay parity。
...
3. **reachability repair gate**：5.9a 的 50/123 只是理论可复用量，不是已实现收益。5.9b 必须报告实际 skeleton builds/hits/stores 与 exact LRU stats；legacy-exact/unsafe 状态必须 0 reuse，wall 只作方向性证据。
