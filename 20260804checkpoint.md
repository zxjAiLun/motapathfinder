...
- **PR-5.5a（完成，研究/证据轮，零 production 改动）**：Multi-Region Candidate-Key Shadow & Boundary Corpus。新增 `lib/multi-region-key-shadow.js`（纯观察 shadow：不参与 frontier pruning / dominance / ranking / checkpoint / route）+ `check-multi-region-key-shadow-contract`。核心问题：**状态经过 Region boundary checkpoint/transfer 后，现有 MT1 candidate identity 是否仍保留所有未来决策相关信息**（Region boundary 本身被视为 successor semantics，不是 state serialization seam）。三层 corpus：pre-boundary（R0 终端候选）/ boundary-transfer（`materializeNextRegionFrontier` 后 R1 真正收到的输入，含 inputCarried/post-boundary exact fp、regionInputIndex、checkpoint fp）/ post-boundary（R1 展开的可达状态）。两条 partition：A. state partition（exact→candidate，按层 + 总体）；B. boundary partition（pre-boundary candidate key → post-boundary exact fp，boundary-transfer equivalence）。有序 CEGAR：boundary-transfer 等价放第一（决定性），再 classifyPair（legal-action/successor/terminal/dominance）。fixture：R0=approved MT1（exp9，多终端候选，HP/位置各异）+ R1=unapproved（production fallback，exp>=4）。负控制（all-colliding key）证明边界检测 fail-visible。**本轮结果：NO_COLLISION_OBSERVED**（3 层全 equal partition，0 split / 0 merge）——只是"未观察到碰撞"，**不是 safe，更不是 multi-Region 认证**。
...
1. **PR-5.5a 已收口**：multi-Region shadow corpus 首轮 NO_COLLISION_OBSERVED（不是 safe）。下一轮 **5.5b — collision CEGAR / minimal refinement**：按用户路线等证据出现 merge 再做字段 refinement/ablation；不预设 regionId/index/entry position/checkpoint fp 就是该加的字段（可能只是另一种 startComponentId）。
2. **multi-Region candidate-key generalization 路线**：5.5a shadow+boundary corpus → 5.5b collision CEGAR/minimal refinement → 5.5c multi-Region workload qualification → 5.5d guarded promotion（只有证据允许时）。
