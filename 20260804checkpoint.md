...
1. **PR-5.5e 已关闭；下一轮进入 reachability reuse attribution**：enumerateActions 内的 walk 是 candidate-default 侧剩余热点；先按 exact input/state/action-scope 量化重复 BFS，再决定 state-local/cacheable projection，不能先改缓存语义。
...
3. **reachability repair gate**：任何 cache/reuse 必须锁 action set、successor exact state、route fingerprint 与 strict replay parity，并报告 hits/misses/stores，而不是只看 wall time。
