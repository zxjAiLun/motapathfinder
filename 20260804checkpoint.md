...
1. **PR-5.8d 已关闭；下一轮 5.8e Recorded Travel-Variant Replay Repair**：根因是 resolver 在 post-state/path 消歧前按 choice fingerprint 去重。修复应继续区分 choice identity 与 travel variant identity，保留同 choice 的全部 variants 直到 exact-post 优先评分完成；补同指纹多 variant 正/负合同，并以 special80→MT8 13 decisions strict replay 作为 closure gate。不得借机改预算、checkpoint、DP key/dominance/selection。
