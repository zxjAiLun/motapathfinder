...
- **PR-5.5b（完成，研究/证据轮，零 production 改动）**：Multi-Region Boundary Corpus Expansion。新增 `check-multi-region-boundary-matrix-contract`（workload matrix 11 个）：R0 frontier 变体（exp2/6/8/9/tileRemoved，**显式 goal，P2-3 关闭**）× entryA；R1 boundary 变体（exp6 固定 R0）× entryA/entryB/entryC（不同入口位置）、inventoryUse（携带 inventory 的 pickup/equip/openDoor/useTool）、flagCarry（battle-only，携带 flags）；3-Region chain（R0→R1→R2，2 条 boundary，验证 per-boundary context 不串）。R1 goal 静态校准（exp 阶梯或 tileRemoved(4,11)）保证 R1 真实搜索。**Coverage gate**（pre≥2 / boundary≥2 / post≥2 / actionKinds≥2）——0 merge 声明必须有覆盖。**4 个检测器级 control**：all-colliding、HP-only dominance-safe、boundary-semantic-drift（Stage 1 立即 unsafe）、post-boundary-action-drift（classifyPair 抓行为差异）。第一次真实 merge → 写 witness artifact（字段级 candidateProjection diff）到 `runs/generated/` + 显式 needs-review（verdict pin `NO_COLLISION_OBSERVED`，禁止静默升级）。**本轮结果：NO_COLLISION_OBSERVED**（11 workload，24 pre / 24 boundary / 106 post 样本，0 merge / 0 unsafe）——证据面显著扩大，但依然不是 safe、不是 multi-Region 认证。
...
1. **PR-5.5b 已收口**：corpus 扩到 11 workload（R0 frontier × 5、R1 boundary × 5、3-Region chain），证据面 24 pre / 24 boundary / 106 post，仍 0 merge。下一轮 **5.5c**：继续扩 corpus（三 Region chain 变体、更多 R1 topology/goal 类型、arrival-event 更强的 floor），直到第一次真实 merge witness；**出现 merge 才**进 collision CEGAR / minimal identity refinement（名字由证据决定）。
...
4. **fast CI <3min**（P2-1 carry）：当前 fast ≈5min（solver-job 95s + route-free 50s + tower-ir 28s + candidate-smoke 104s）。candidate-key-smoke 实际仍跑 4 次 solve（strict A/B + timed A/B）。要达标需在 fast 内部按分支并行，wall = max(各分支)。建议独立 CI-INFRA PR。
5. **DIAG-HYGIENE carry**：paired benchmark 旧 control `candidateProductionProfileDefaultOff:true` 与 PR-5.4f 后默认语义不符，改为 `approvedMt1CandidateDefaultOn` + `explicitProductionRollbackAvailable` 或删除；与 CI-INFRA 一起修，不塞进 solver 主线。
