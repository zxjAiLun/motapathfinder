...
1. **PR-5.8c 已关闭；下一轮 5.8d MT7→MT8 Strict Replay Attribution**：左剑已证明 found/replay 成功，500 exhaustion 只是 incomplete skyline。下一步从 exact special80 state 对 long-horizon winner 的 decision 13 post-state mismatch 做最小 A/B，先审计 route prefix、compound lineage、recorded pre/post state 与 simulator replay；未归因前不得宣称 MT8 closure、扩大预算或修改 DP key/dominance/selection。
