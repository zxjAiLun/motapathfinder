...
1. **PR-5.7 已关闭；下一轮 5.8a Post-MT5 Long-Chain Baseline**：从 `mt5-blueking-kill` 继续跑现有 `MT6 → MT7 → MT8 entry` milestone graph，先用固定 expansion / 无 wall timeout 建立首个失败点与 strict replay 基线，再决定是否需要资源时序 checkpoint。MT5 当前已不是 correctness blocker；`mt5-early-gem-entry` 的 133-expansion branch 仅是后续性能候选，禁止为它扩大 timeout/candidateLimit、修改 DP key 或引入 OnlyUp-specific core heuristic。
