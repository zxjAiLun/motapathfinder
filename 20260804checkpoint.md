...
- **PR-5.5c Cross-Topology / Inventory Collision Hunt（完成，研究/证据轮，零 production 行为改动）**：攻击真实 union 证明的两个空洞。扫描真实 tower 后确认 OnlyUp MT1 无 door/key/drop/changeFloor（inventory/visitedFloors 在 MT1 语义恒定），sample floor 是可破坏墙迷宫（搜索爆炸），改用 **whiteisland（9）真实第二塔**：新增 `wi-a1-door-key-entryA` workload（R0 approved OnlyUp MT1 exp2 → R1 whiteisland A1，floor entry (5,1)，goal 开门 tileRemoved(5,3)）——R1 必须真实捡 yellow key 并开门，**inventory 真实 acquire→consume（distinct 11：yellowKey 0-5/blueKey）**，door 打开产生 284 种 floor mutation；**跨塔 boundary（MT1→A1）携带 MT1 历史并执行 A1 arrival semantics，状态跨 2 floor（visitedFloors=2）**。为此 contract 支持 **per-region project**（regionContext 用 region 自己的 tower 加载 simulator/TowerIR/milestone/legal provider；lib `buildCorpusRecord` 用 `regionContext.project` 算 candidate key）。顺带修正：legal kinds 从 action 对象 `.kind` 提取（summary 前缀如 getNext/event@MT1 是格式 artifact）；raw-vs-legal summary parity 移除（provider 即 production 语义，raw 差异是 policy 过滤的固有部分，由 rawVsLegalControl 覆盖）。**结果：NO_COLLISION_OBSERVED**（19 workload，44 pre / 44 boundary / **1342 post**，0 merge / 0 unsafe）——全局 semantic union：inventory **11**（原 1）、visitedFloors **2**（原 1）、regionKey 245、legalActionSet 478；**两个覆盖空洞被真实证据补齐，依然无同 scope candidate collision**。
