...
1. **PR-5.9b 已关闭；下一轮进入 PR-5.9c reachability rebase cost attribution**：量化 6649 clone / 6526 dominance-key builds 分别由哪些 reachability consumers 实际需要，先观测后决定 lazy/materialization boundary；不得直接共享 state 或跳过 current-state rebase。
...
3. **reachability repair gate**：5.9b 已兑现 50/123 skeleton reuse；后续不应再用扩大 cache key 猜优化。任何 lazy rebase 必须保持 unsafe 0 reuse、逐 exact safety classification、action/travel/successor exact corpus与 strict replay，wall 只作方向性证据。
