...
1. **PR-5.4f 已收口**：MT1 默认 profile = 候选（approved scope），`production-region` 为显式 rollback。MT1_DEFAULT_PROMOTION_ACCEPTED。下一轮可做全塔回归验证 promotion 无扩散，或推进 multi-Region / CompactState / Rust core。
...
4. **fast CI <3min**（P2-1 carry）：当前 fast ≈5min（solver-job 95s + route-free 50s + tower-ir 28s + candidate-smoke 104s）。candidate-key-smoke 实际仍跑 4 次 solve（strict A/B + timed A/B）。要达标需在 fast 内部按分支并行，wall = max(各分支)。
