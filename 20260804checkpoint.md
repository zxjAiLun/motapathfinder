...
24. **PR-5.17a13 deferred-resource attribution（已完成）**：搜索已能发现 I621，但把它误写成“现在获取”的 repair；成功所需的是“现在保留、越过成本链、以后获取”。后续不得用加 expansions 或继续优先 EXP 掩盖 vocabulary 缺口；5.17a14 必须自动生成并严格回放三阶段时序事务，且 oracle route 仍只能参与修复后的评价。
