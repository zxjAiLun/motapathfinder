...
1. **PR-5.5a Repair 已收口**：corpus machinery 已按 review 修正（per-region IR、真实 floor entry、semantic-identity boundary gate、verdict pin）。下一轮 **5.5b — collision CEGAR / minimal refinement**：先扩 5.5a corpus 到真正非平凡 boundary（更多行为类型），只有出现真实 merge witness 后才做字段 refinement/ablation；不预设 regionId/index/entry position/checkpoint fp 就是该加的字段（可能只是另一种 startComponentId）。
