...
1. **PR-5.8e 已关闭；下一轮 5.8f Solver Outcome Taxonomy / Doctor Hygiene**：永久拆分 `goalFound/frontierExhausted/budgetExhausted/searchComplete`；明确 `goalFound=true && searchComplete=false` 不是 failure/not-found。只改 doctor/report/schema 与回归合同，不改变搜索行为。
