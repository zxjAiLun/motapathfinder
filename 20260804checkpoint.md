...
- **PR-5.5c（完成，研究/证据轮，零 production 行为改动）**：Multi-Region Corpus Expansion（找第一次真实 collision）。`check-multi-region-boundary-matrix-contract` 扩到 **18 workload**：新增 cross-products（r0-exp2/exp9/tile4_1 × entryB/entryC/inventoryUse/flagCarry，变换 pre-boundary 终端多样性 × R1 结构）、deep R1 搜索（exp6-entryA-deep：candidateLimit 4 / goalSkylineLimit 16，post-boundary 26 样本）、**4-Region chain**（R0→R1→R2→R3，3 条 boundary）。R1 goal 静态校准不变（changeFloor/MT2 topology 变体实测不可达/平凡后弃用）。**本轮结果：NO_COLLISION_OBSERVED**（18 workload，42 pre / 42 boundary / 189 post 样本，0 merge / 0 unsafe）——证据面从 24/24/106 扩大到 42/42/189，仍未抓到真实同 scope candidate collision；不是 safe、不是 multi-Region 认证。
...
1. **PR-5.5c 已收口**：corpus 扩到 18 workload（42/42/189），仍 0 真实 collision。下一轮 **5.5c 续 / 5.5d**：继续扩 corpus（更多 chain 变体 / 不同 tower 的 region / arrival 强 floor / 更深的 R1 budget），直到第一次真实同 scope candidate collision；出现 merge 立即停，按 witness 分叉：行为等价/dominance 安全 → CEGAR safety proof；行为不同 → minimal identity refinement。
...
4. **fast CI <3min**（P2-1 carry）：当前 fast ≈4m29s（solver-job / route-free / candidate-smoke 串行主导）。要达标需在 fast 内部按分支并行，wall = max(各分支)。建议独立 CI-INFRA PR。
