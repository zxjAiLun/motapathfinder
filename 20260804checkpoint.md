...
- **PR-5.8b MT6 Defense Timing Causal Repair（完成，提交见本文件 HEAD）**：最小反事实只前移 `MT6:2,1` checkpoint 并保护 `MT6:6,8`，未受影响 milestones 完全一致，affected DP/action scope 及全局 candidateLimit 8、500 expansions/segment、`maxRuntimeMs=0`、goal-directed 全部不变；production key/dominance/selection 不变。5.8a historical A 仍复现 **362 / special80 FAIL / route 14ca77de…**；repair B 为 **508 / special80 REACHED / 17045ms**，hero `hp2672845/atk5767/def5535/mdef30010/exp881`，24 decisions strict replay（route `e0ea77ee…`，state `0564b870200d0113`）。verdict `CAUSAL_ROOT_CAUSE`。继续到 MT8 的 probe 在 `mt7-left-sword` 命中 500-expansion budget，0 trimming、无 wall timeout，故只记 `INCOMPLETE_WITHIN_BUDGET`。
...
1. **PR-5.8b 已关闭；下一轮 5.8c MT7 Left-Sword Budget Attribution**：从 strict special80 state 复现 `mt7-left-sword` 的 500-expansion exhaustion，先读 doctor/raw failure 的 actionTrimmed、frontier、goal fields、资源时序和 repair attempts；未完成归因前不得扩大 timeout/candidateLimit/expansions、修改 DP key/dominance/selection，或引入 OnlyUp-specific core heuristic。
2. **multi-Region candidate-key 研究线保持独立**：5.5e visitedFloors/changeFloor surface → 5.5f collision CEGAR/minimal refinement（只有证据允许时），不得借 5.8 长链搜索扩大 MT1 production key scope。
