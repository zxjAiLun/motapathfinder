...
1. **PR-5.8 系列已关闭；下一轮回到 5.5e visitedFloors/changeFloor surface**：只用 tracked、same-project、production-faithful parent→action→child changeFloor evidence 补齐 PARTIAL hole；证据不足继续 PARTIAL，不得用 boundary arrival 冒充真实 changeFloor。
2. **5.5f collision CEGAR/minimal refinement 仍需 evidence gate**：只有 5.5e 产生真实 semantic collision 才进入 refinement；`NO_COLLISION_OBSERVED` 仍不等于 safe/promotion candidate。
