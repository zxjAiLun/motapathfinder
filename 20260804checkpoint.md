...
1. **PR-5.8a 已关闭；下一轮 5.8b MT6 Defense Resource-Timing Refinement**：按已证实顺序把 `MT6:2,1` +500 DEF 前移到 `MT6:6,8` 之前，再细化 `9,10 → 11,11 → final mdef → MT7 bottom pair → redSwordsman/I619 → special80` checkpoint；用相同 tracked fixture、500 expansions/segment、`maxRuntimeMs=0` 对比 5.8a 的 **362 post-MT5 / 1007 total / special80 FAIL**。不得扩大 timeout/candidateLimit、修改 DP key/dominance/selection，或引入 OnlyUp-specific core heuristic。
