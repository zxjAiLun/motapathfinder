...
1. **PR-5.9c 已关闭；下一轮进入 PR-5.9d topology-first materialization**：先把 safe-fast adjacency/tile lookup 改用 base state + skeleton coordinate/path，再仅为 stateful candidate/action 构造 current-state travel state；不得用 lazy getter，因为当前 API 会读取所有 nodes。legacy-exact 保持原实现。
...
3. **reachability repair gate**：5.9d 必须在同一 checker 报告 materialized nodes/clone/key builds 从 5.9c baseline 的实际下降，并保持 unsafe 0 reuse、逐 exact safety classification、116-state/434 action+successor exact corpus与 strict replay；wall 只作方向性证据。
