...
1. **PR-5.4g 已收口**：promotion 后系统级回归通过（S1–S4），MT1_DEFAULT_PROMOTION_REGIME_STABLE。发现并修复 projectRoot 归一化一致性 + v2 per-region resolution。下一轮可推进 multi-Region candidate-key generalization。
2. **multi-Region candidate-key generalization**（下一能力主线）：回答"已证明的 MT1 state identity 能否安全跨 Region 边界泛化"——前提是已把 approved MT1 scope 的 default 打稳。
...
5. **production 时间预算非确定性**：`dpBudget.maxRuntimeMs` 下的搜索结果依赖机器速度，需要确定性 budget 语义（按 expansion 而非 wall time）或文档化非确定。
6. **CompactState / Rust core**：在 multi-Region key 研究后再推进。
