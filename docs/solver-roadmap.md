...
- ✅ PR-4.6a1 使用 deterministic synthetic simulator 真实执行 `runAdaptiveSegmentPlanner(maxAdaptiveRepairs=1)`；observed outcome 不再来自 fixture 常量。
...
- `target-action-unreachable`：先使用已验证的 adaptive window/change-floor repair；清 blocker / 开门白名单需等实际 planner evidence 后再扩展。
...
```bash
npm run check:region:entry:contract --prefix shared-solver
...
#### PR-5.2c：Resume Status Retention and Play State Controls ✅ route-gui-resume-status-controls

- ✅ failed gate/restore 后的 `lastRuntimeStatus` 不会因后续异步 status poll 在 runtime 已关闭时被清空；非空 runtime status 仍会正常刷新缓存。
- ✅ 真实 GUI API checker exact-lock `busy`、`completed` 和 `failed` 状态下的 `POST /api/resume/play` 为 HTTP `409`，分别覆盖 `REPLAY_RESUME_BUSY` 与 `REPLAY_RESUME_INVALID_STATE`。
- ✅ 本轮只补 resume operation 的诊断与边界控制，不修改 production DP key、dominance、agenda、容量、默认策略、路线选择或既有路线 `ReplaySession` 语义。
...
### 2026-08-04 更新：PR-5.3a Manual Solver Model Contract & Compact State Projection
...
```bash
npm run check:solver-model --prefix shared-solver
npm run check:region-specs --prefix shared-solver
npm run check:region:route:contract --prefix shared-solver
npm run check:static --prefix shared-solver
```
...
- `launcher/server.js` + `router.js` + `job-api.js` + `tower-registry.js` + `run-solver-launcher.js`：Node 内置 HTTP，默认 localhost-only；Tower Registry 服务端校验项目/fingerprint/region 列表并拒绝路径穿越。
...
### 2026-08-04 更新：PR-5.3d2 Repair Budget Authority & Per-attempt Progress
