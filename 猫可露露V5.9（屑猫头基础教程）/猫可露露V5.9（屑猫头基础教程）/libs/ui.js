...
ui.prototype._drawBook_drawDamage = function (index, enemy, offset, position) {
    ...
    //标记符号
        ...
        if (core.enemys.hasSpecial(enemy, 113)) damage += "⤨"; 
        if (core.enemys.hasSpecial(enemy, 85)) damage += "☉"; 
        if (core.enemys.hasSpecial(enemy, 86)) damage += "≈"; 
        if (core.enemys.hasSpecial(enemy, 88)) damage += "♋"; 
        if (core.enemys.hasSpecial(enemy, 89)) damage += "♆"; 
        if (core.enemys.hasSpecial(enemy, 114)) damage += "☁️";      
        if (core.enemys.hasSpecial(enemy, 120)) damage += "♢"; 
        if (core.enemys.hasSpecial(enemy, 121)) damage += "♡"; 
        if (core.enemys.hasSpecial(enemy, 141))damage += "✿";
        if (core.enemys.hasSpecial(enemy, 142))damage += "☀";
        if (core.enemys.hasSpecial(enemy, 143))damage += "▣";
        if (core.enemys.hasSpecial(enemy, 145))damage += "☒";
        if (core.enemys.hasSpecial(enemy, 146))damage += "☢";
        if (core.enemys.hasSpecial(enemy, 158))damage += "☢";
        if (core.enemys.hasSpecial(enemy, 153)) damage += "♬";  
        if (core.enemys.hasSpecial(enemy, 154)) damage += "♬";  
        if (core.enemys.hasSpecial(enemy, 156)) damage += "◀";  
        if (core.enemys.hasSpecial(enemy, 157)) damage += "☁️";  
        if (core.enemys.hasSpecial(enemy, 26)) damage += "☸";          
    }
...
ui.prototype._drawStatistics = function (floorIds) {
    ...
    core.drawText([
        ...
        +"总计打死了"+statistics.battle+"个怪物，得到了"+core.formatBigNumber(statistics.money)+"领悟，"+core.formatBigNumber(statistics.exp)+"点经验。\n\n"
...
ui.prototype._drawStatistics_generateText = function (obj, type, data) {
    ...
    if (core.flags.statusBarItems.indexOf('enableMoney')>=0) text+="，总领悟数"+data.monster.money;
