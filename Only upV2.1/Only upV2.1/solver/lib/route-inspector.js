...
function buildDecisionRows(routeRecord, project) {
  return (routeRecord.decisions || []).map((decision, offset) => {
    ...
    return {
      ...
      enemyName: lookupName(project, "enemy", decision.enemyId) || null,
      ...
      mapThing: buildMapThing(project, decision),
      thingMismatch: decision.enemyId && buildMapThing(project, decision) && buildMapThing(project, decision).id !== decision.enemyId ? true : false,
...
function buildStepDetail(routeRecord, index, project) {
  ...
  displayDecision.enemyName = lookupName(project, "enemy", decision.enemyId) || null;
